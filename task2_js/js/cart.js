...
const add_func = (p) => {
  ...
  if (
    ...
    alert(
      "there is an error , please check the value entered again !"
    );
    return "";
  }
};
const add_row = (p, i) => {
  ...
  return `<tr class="thead-dark"><td>${p.productName}</td><td>${p.productPrice}AED</td><td><div class="numOfProducts"><button type="button"  onclick="decQuantity(${i})">
<i class="fa fa-minus"></i>
</button>${p.productNumber} 
<button type="button"  onclick="incQuantity(${i})">
    <i class="fa fa-plus"></i>
</button></div>
</td><td>${producttotal}AED</td><td><button onclick="delete_func(${i})">remove</button></td></tr>`;
};
const subtotal = () => {
  let st = product
